...
description: |
  模拟新用户实机走 BotLearn 课程平台（course.botlearn.com.cn，另有测试环境），采集真实产品截图，并排版成对外运营物料——学员上手手册/长图、产品功能海报、发群指导图。只要用户想要「课程平台的真实截图」或「基于真实截图的物料」，即使没说"截图"二字也应触发：做上手手册、上手长图、新学员指引、把产品界面做成海报、更新手册里的截图、给某功能拍个图。
...
| 测试 | 地址向 Susan 索取（不写进公开仓） | 新功能抢先拍、可随意折腾 | 每次现向 Susan 要测试码 |
